...
function parseTruthy(val: Value): boolean {
    return !(
        (val.type == Type.NULL)
...
export function execute(main: Buffer, ftab: Array<Function>) {
    let ctx = new Context(main);
        ...
        switch (op) {
            ...
            case Op.CALL: {
                ...
                if (func.type == Type.FUNCTION) {
                    ...
                    ctx.ip = 0;
                    ctx.switchCode((func.v as Function).code);
                }
                else {
                    let args: Value[] = [];
